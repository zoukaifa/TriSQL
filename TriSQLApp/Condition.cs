...
namespace TriSQLApp
{
    ...
    class ConditionPair
    {
        ...
        public List<int> rightValueListint;
        ...
        public ConditionPair(List<int> column, string left, Operand operand, RightOptType rightIsValue)
        {
            //Console.WriteLine("带参数初始化构造函数构造成功(有嵌套)");
            this.left = left;
            this.operand = operand;
            ...
            //this.rightTable = table;
            this.rightValueListint = column; //需增加一个异常捕获
        }
    ...
    class Condition
    {
        ...
        //初始构造函数
        public Condition(Table table, string condition, List<int> nestedColumn = null)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < strs.Length; i++)
                {
                    ...
                    Console.WriteLine(strs[i].Equals("in"));
                    ...
                    if (strs[i].Equals("in"))
                    {
                        ...
                        this.conditions.Add(new ConditionPair(nestedColumn, param1, opt, type));
        ...
        {
            int isDefaultLocal = this.isDefault;
            ...
            else if (isDefaultLocal == 0)
            {
                //Console.WriteLine("默认查询表全部内容");
                return true;
            }

            if (this.strs == null || this.strs.Count() == 0)
            {
                new Exception("strs为空");
            }

            string[] strsLocal = (string[])this.strs.Clone();
            ...
            for (int i = 0; i < curConditon.Count; i++)
            {
                ...
                {
                    for (int j = 0; j < cp.rightValueListint.Count; j++)
                    {
                        if (leftValue.Equals(cp.rightValueListint[j]))
                        {
                            isMatch = true;
                            break;
                        }
            ...
            for (int i = 0; i < strsLocal.Length; i++)
            {
                if (strsLocal[i].Equals("?"))
                {
                    strsLocal[i] = cmpResult[count++] == true ? "1" : "0";
                }
                sb.Append(strsLocal[i]);
            }
